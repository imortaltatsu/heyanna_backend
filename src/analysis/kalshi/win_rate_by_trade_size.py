...
class WinRateByTradeSizeAnalysis(Analysis):
    ...
    def run(self) -> AnalysisOutput:
        ...
        # matplotlib figures is expensive and can be fragile in headless or
        # concurrent environments, so we skip figure generation here to keep
        # responses fast and robust.
        chart = self._create_chart(df)

        return AnalysisOutput(figure=None, data=df, chart=chart)
