...
async def get_dashboard() -> DashboardResponse:
    ...
    # Fetch all analyses concurrently; most will be cache hits when the
    # background analysis worker is running.
    (
        meta_entry,
        win_rate_entry,
        returns_entry,
        ...
        cache.get_or_refresh("meta_stats"),
        cache.get_or_refresh("win_rate_by_price"),
        cache.get_or_refresh("returns_by_hour"),
        cache.get_or_refresh("volume_over_time"),
        cache.get_or_refresh("maker_taker_gap_over_time"),
        cache.get_or_refresh("ev_yes_vs_no"),
        cache.get_or_refresh("mispricing_by_price"),
        cache.get_or_refresh("longshot_volume_share_over_time"),
        cache.get_or_refresh("trade_size_by_role"),
        cache.get_or_refresh("maker_vs_taker_returns"),
        cache.get_or_refresh("maker_returns_by_direction"),
        cache.get_or_refresh("maker_win_rate_by_direction"),
        cache.get_or_refresh("yes_vs_no_by_price"),
        cache.get_or_refresh("maker_taker_returns_by_category"),
        cache.get_or_refresh("win_rate_by_trade_size"),
        cache.get_or_refresh("kalshi_calibration_deviation_over_time"),
    )

    meta_data = meta_entry.data.get("data", [])
    metrics = {row.get("metric"): row.get("value") for row in meta_data}

    all_entries = [
        meta_entry, win_rate_entry, returns_entry, volume_entry,
        maker_taker_entry, ev_entry, mispricing_entry, longshot_entry,
        trade_size_entry, maker_vs_taker_entry, maker_direction_entry,
        maker_win_entry, yes_no_entry, category_entry, trade_size_win_entry,
        calibration_entry
    ]
    min_created_at = min(r.created_at for r in all_entries)
    ...
    return DashboardResponse(
        ...
        maker_taker_returns_by_category={
            "data": category_entry.data.get("data", []),
            "chart": category_entry.data.get("chart"),
        },
        win_rate_by_trade_size={
            "data": trade_size_win_entry.data.get("data", []),
            "chart": trade_size_win_entry.data.get("chart"),
        },
        kalshi_calibration_deviation={
            "data": calibration_entry.data.get("data", []),
            "chart": calibration_entry.data.get("chart"),
        },
...
async def get_win_rate_by_price() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("win_rate_by_price")
...
async def get_returns_by_hour() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("returns_by_hour")
    return AnalysisResponse(
...
async def get_volume_over_time() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("volume_over_time")
    return AnalysisResponse(
...
async def get_maker_taker_gap() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("maker_taker_gap_over_time")
    return AnalysisResponse(
...
async def get_ev_yes_vs_no() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("ev_yes_vs_no")
    return AnalysisResponse(
        name="ev_yes_vs_no",
...
async def get_mispricing_by_price() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("mispricing_by_price")
    return AnalysisResponse(
...
async def get_longshot_volume_share() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("longshot_volume_share_over_time")
    return AnalysisResponse(
...
async def get_trade_size_by_role() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("trade_size_by_role")
    return AnalysisResponse(
...
async def get_maker_vs_taker_returns() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("maker_vs_taker_returns")
    return AnalysisResponse(
...
async def get_maker_returns_by_direction() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("maker_returns_by_direction")
    return AnalysisResponse(
...
async def get_maker_win_rate_by_direction() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("maker_win_rate_by_direction")
    return AnalysisResponse(
...
async def get_yes_vs_no_by_price() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("yes_vs_no_by_price")
    return AnalysisResponse(
...
async def get_maker_taker_returns_by_category() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("maker_taker_returns_by_category")
    return AnalysisResponse(
...
async def get_win_rate_by_trade_size() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("win_rate_by_trade_size")
    return AnalysisResponse(
...
async def get_kalshi_calibration_deviation() -> AnalysisResponse:
    ...
    """
    cache = get_cache_instance()
    entry = await cache.get_or_refresh("kalshi_calibration_deviation_over_time")
    return AnalysisResponse(
...
async def get_analysis(analysis_name: str) -> AnalysisResponse:
    ...
    entry = await cache.get_or_refresh(analysis_name)
    return AnalysisResponse(
        name=analysis_name,
